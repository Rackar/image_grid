...
function readShapeFile() {
  ...
  shp("./myshapes/test").then(
    async function (geojson) {
      ...
      let array = optimizeImages(allBackupGrids);
      groupImagesToShp(array);
    },
    (e) => {
      console.log(e);
    }
...
function optimizeImages(arr) {
  if (arr && arr.length) {
    ...
    backupArrs.forEach((grid) => {
      ...
      backupHash[grid.gridId].push({
        q: grid.previousFilename,
        h: grid.filename,
        filename: grid.filename,
        previousFilename: grid.previousFilename,
        acquisitio: grid.acquisitio,
        status: grid.status,
      });
    });
    ...
    for (const key in backupHash) {
      if (Object.hasOwnProperty.call(backupHash, key)) {
        ...
        if (sortedArr.length === 0) sortedArr.push({ info: obj, gridId: key });
        //倒序排列，可能不需要
        let importIndex = 0;
    ...
    let sureImage = sortedArr.filter((grid) => grid.info.length === 1);
